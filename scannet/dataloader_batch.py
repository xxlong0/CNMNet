...
class ScannetDataset(Dataset):
    ...
    def load_single_sample(self, scene_id, image_id):
        ...
        normal_filepath = os.path.join(os.path.join(scene_id, 'lg_normal'), image_id + '.npy')
        ...
        try:
            normal = np.load(normal_filepath)
            normal = np.float32(normal)
        except:
            ## normal is png
            normal = self.load_normal_png(normal_filepath.replace("npy", "png"))
